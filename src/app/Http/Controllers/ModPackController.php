...
class ModPackController extends Controller
{
    ...
    public function createRun(Request $request, string $id)
    {
        ...
        if (! $loaderDownloaded) {
            ...
            // Write a basic server.properties file to avoid benign startup errors
            $serverPropertiesPath = $runDir.'/server.properties';
            if (! file_exists($serverPropertiesPath)) {
                $defaultServerProperties = implode("\n", [
                    ...
                    'generator-settings={}',
                    'level-name=world',
                    'motd=EasyCurse Virtual Test Server',
                    'query.port=25565',
                    'pvp=true',
                ...
                $serverPropertiesWritten = file_put_contents(
                    $serverPropertiesPath,
                    $defaultServerProperties,
                );

                if ($serverPropertiesWritten === false) {
                    \Log::warning('Failed to write server.properties for virtual run', [
                        'run_id' => $run->id,
                        'run_dir' => $runDir,
                    ]);
                }
